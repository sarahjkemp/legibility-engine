...
async def run(target: AuditTarget, config: AuditConfig, settings: EngineSettings) -> SubScoreResult:
    ...
    for body in found:
        ...
        verified_results = await verify_entity_matches(results, profile, settings)
        if verified_results:
            verified.append({"body": body, "result": verified_results[0]})
    count = len(verified)
    ...
    return SubScoreResult(
        ...
        raw_data={"found_bodies": found, "verified": verified, "entity_profile": profile.__dict__},
    )
