...
async def run(target: AuditTarget, config: AuditConfig, settings: EngineSettings) -> SubScoreResult:
    ...
    platform_surfaces = await fetch_owned_channel_surfaces(target, settings)
    platform_candidates = [
        item
        for item in platform_surfaces
        if item["platform"] != "website"
        if any(
            term in (((item.get("title") or "") + " " + (item.get("text") or "")).lower())
            for term in ["client", "case", "result", "growth", "podcast", "interview"]
        )
    ]
    if not candidate_pages and not platform_candidates:
        return SubScoreResult(score=0.0, confidence=0.6, findings=[SubScoreFinding(severity="medium", text="No obvious case study, client proof, or declared channel evidence surfaces were found.")], raw_data={"candidate_pages": [], "platform_candidates": []})
    strong = []
    ...
    strong_platform = [
        ...
        if any(term in ((item.get("text") or "") + " " + (item.get("title") or "")).lower() for term in ["client", "%", "growth", "result", "case study", "interview"])
    ]
    ...
    return SubScoreResult(
        ...
        + [evidence(item["url"], item.get("text", "")[:180] or item.get("title") or "") for item in strong_platform[:8]],
        ...
        raw_data={"candidate_pages": [page["url"] for page in candidate_pages], "strong_pages": [page["url"] for page in strong], "platform_surfaces": [item["url"] for item in platform_surfaces], "platform_candidates": [item["url"] for item in platform_candidates], "strong_platform": [item["url"] for item in strong_platform]},
    )
