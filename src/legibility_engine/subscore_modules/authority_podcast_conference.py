...
from ..collectors.search import dedupe_by_registered_domain, search_web, verify_brand_matches
...
async def run(target: AuditTarget, config: AuditConfig, settings: EngineSettings) -> SubScoreResult:
    ...
    combined = await verify_brand_matches(podcast_hits + speaker_hits, target.company_name, settings)
