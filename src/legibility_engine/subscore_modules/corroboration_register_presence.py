...
async def run(target: AuditTarget, config: AuditConfig, settings: EngineSettings) -> SubScoreResult:
    ...
    profile = await fetch_company_profile(companies[0]["url"], settings) if companies else None
    wikidata = await lookup_entity(target.company_name, settings, limit=3)
    linkedin_hits = await search_web(f'site:linkedin.com/company "{target.company_name}"', settings, limit=3)
    crunchbase_hits = await search_web(f'site:crunchbase.com/organization "{target.company_name}"', settings, limit=3)
    ...
    if profile:
        evidence_items.append(evidence(profile["url"], profile.get("registered_address") or profile.get("page_text", "")[:200]))
    if wikidata:
    ...
    return SubScoreResult(
        ...
        raw_data={
            ...
            "companies_house_profile": profile,
            "wikidata": wikidata,
